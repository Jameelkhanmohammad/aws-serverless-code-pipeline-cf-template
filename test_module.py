...
class TestPolicies(unittest.TestCase):
    def test_policy(self):
        ...
        policy2 = readFile("iam_policy.json")
        actions = json.loads(readFile("actions.json"))
        source = readFile("source.txt")

        evaluation_results = simulatePrincipalPolicy(source, actions, [policy,policy2])
        # evaluation_results = simulateCustomPolicy( actions, [policy])
